...
class Topartist extends Component {
  ...
  async getTopArtists(time) {
    try {
      const response = await spotifyWebApi.getMyTopArtists({ limit: 50, time_range: time });
      this.getUserOption(time);
  
      // Update the state with the artist objects including the follower count
      this.setState({
        nowList: {
...
getUserOption(time) {
  ...
  WordList(props) {
    ...
    const items = words.map((artist, idx) => (
      <div key={idx} className="col-lg-4 col-md-6 portfolio-item filter-app">
        <div className="portfolio-wrap">
          <img src={artist.images[0].url} className="img-album img-fluid" alt="" />
          <div className="portfolio-info">
            <h4>{idx + 1}. {artist.name}</h4>
            <p>{artist.genres[0]} </p>
            <div className="portfolio-links">
              <a href={artist.external_urls.spotify} target="_blank" rel="noopener noreferrer" title="VISIT ON SPOTIFY">
